...
func Connect(mode *serial.Mode, logger *log.Logger, port string) (a Arduino) {
	...
	logger.Printf("Starting arduinno connect on Port: %s, with baudrate: %d\n", port, mode.BaudRate)
	fmt.Printf("Starting arduinno connect on Port: %s, with baudrate: %d\n", port, mode.BaudRate)
...
func (a Arduino) Dataload(logger *log.Logger, data string) error {
	...
	logger.Printf("On Arduino was write %d bytes and message: %s", n, data)
	fmt.Printf("On Arduino was write %d bytes and message: %s", n, data)
	return nil
}
